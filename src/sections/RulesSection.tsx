...
export const RulesSection: React.FC = () => {
  ...
  const getRuleBadge = (rule: Rule) => {
    if (rule.status === 'INACTIVE') return 'bg-slate-100 text-slate-600';
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-slate-900">{rules.filter(r => r.deviceId).length}</p>
            </div>
          </div>
        </div>
      </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg ${
                          rule.status === 'ACTIVE' ? 'bg-green-50' : 'bg-slate-100'
                        }`}>
                          {getRuleIcon(rule)}
                        </div>
                        <div>
